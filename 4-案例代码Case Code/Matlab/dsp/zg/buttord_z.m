function [order,wn] = buttord_z(wp,ws,rp,rs,opt)
...
%modified by Zhiguo Zhou 2019.5.3
%W01 < W0 < W02
W0_P = WP / ( (10^(.1*abs(rp)) - 1)^(1/(2*(abs(order)))));%Í¨´ø
W0_S = WS / ( (10^(.1*abs(rs)) - 1)^(1/(2*(abs(order)))));%×è´ø
